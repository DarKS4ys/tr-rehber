...
export default function Header({ header }: { header: any }) {
  ...
  const backgroundYFadeOpacity = useTransform(
    scrollYProgress,
    [0.65, 0.85],
    ['100%', '0%']
  );
  const textYFadeOpacity = useTransform(
    scrollYProgress,
    [0.6, 0.7],
    ['100%', '0%']
  );
  ...
  return (
    <div
      id="homepage"
      ref={ref}
      className="overflow-hidden h-screen w-full items-center flex flex-col py-[24vh] md:py-[12vh] 2xl:py-40 text-lg relative text-center p-4"
    >
      <motion.div
        className="z-20 relative"
        style={{ y: textY, scale: scaleTransform, opacity: textYFadeOpacity }}
      >
